...
X = dataset.iloc[:1933,reqdCols].values
...
def push_to_dataQ(sensor_data):
    ...
    if sensor_data != "End":
        ...
        window_data_train["data"] = np.array(sensor_data[:])[:,:-1].tolist()#sensor_data[:]#np.array(sensor_data[:])[:][:-1].tolist()[:]
        #print("without label:", np.array(sensor_data[:])[:,:-1].tolist()[:])
    else:
