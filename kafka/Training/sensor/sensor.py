...
class sensor:
    def __init__(self, config_data):
        ...
        self.files = config_data["dataset"]["files"]
        self.isFileBreakEnabled = config_data["dataset"]["isFileBreakEnabled"]
        self.datasetObj = dataset({
                                "chunkSize": self.chunk_size, #self.datasetChunkSize, 
                                "files": self.files, 
                                "isFileBreakEnabled": self.isFileBreakEnabled #"cols": [self.timestampCol, self.labelCol],
                            })
        
        self.labelDataChunkSize = config_data["labelExtr"]["chunkSize"] #config_data["labelDataChunkSize"]
        self.labelDatasetFiles = config_data["labelExtr"]["files"]
        self.labelCol = config_data["labelExtr"]["labelCol"]
        self.timestampCol = config_data["labelExtr"]["timestampCol"]
        self.labelExtObj = labelExtractor({
                                "datasetChunkSize":self.labelDataChunkSize, 
                                "files" : self.labelDatasetFiles,
                                "labelCol": self.labelCol,
                                "timestampCol": self.timestampCol,
                                "isFileBreakEnabled": self.isFileBreakEnabled
                            })

        self.curDataChunk = self.datasetObj.fetchNextChunk().iloc[:, self.reqd_cols].values
        self.curDataChunkSize = len(self.curDataChunk)
        (self.labelStartTs, self.labelEndTs, self.label, self.allChunksParsed) = self.labelExtObj.fetchLabel()
        self.rowOffset = 0
    ...
    def push_to_dataQ(self, sensor_data):
        ...
        if sensor_data["data"] != "End":
            self.window_data["data"] = np.array(sensor_data["data"]).tolist()
            self.window_data["label"] = sensor_data["label"]
        else:
            self.window_data["label"] = -1
    ...
    def isValidLabel(self, ts):
        return ts >= self.labelStartTs and ts <= self.labelEndTs

    def fetch_cur_window_and_slide_V2(self):
        # check if segmented window is present in queue
        if len(self.data_windows_list):
            response = self.data_windows_list.pop(0)
            print("Window fetched from queue:", response)
            return response

        # check if reqd no. of rows have been parsed
        while self.totalRowsParsed < self.rows_limit:

            print(self.dataset_read_completed, len(self.data_list))

            # check if new chunk needs to be fetched
            while self.curDataChunk[self.curDataChunkSize-1][0] < self.labelStartTs:
                
                self.totalRowsParsed += (self.curDataChunkSize - self.rowOffset)
                if self.totalRowsParsed >= self.rows_limit:
                    self.dataset_read_completed = True #optional
                    return [] 
            ...
            # check if cur row needs to be skipped
            while self.curDataChunk[self.rowOffset][0] < self.labelStartTs:
                
                self.rowOffset += 1
                self.totalRowsParsed += 1
                
                if self.totalRowsParsed >= self.rows_limit:
                    self.dataset_read_completed = True
                    return []
                
                if self.rowOffset >= self.curDataChunkSize:
                    try:
                        self.curDataChunk = self.datasetObj.fetchNextChunk().iloc[:, self.reqd_cols].values
                    except:
                        # check if all files have been parsed
            ...
            # check if label needs to be fetched
            while self.labelEndTs < self.curDataChunk[self.rowOffset][0]:
                (self.labelStartTs, self.labelEndTs, self.label, self.allChunksParsed) = self.labelExtObj.fetchLabel()

            self.base_TS = (self.curDataChunk[self.rowOffset][0] // self.window_size) * self.window_size
            self.base_TS = (self.base_TS + self.step_size) if (self.base_TS + self.step_size) <= self.curDataChunk[self.rowOffset][0] else self.base_TS

            while self.curDataChunk[self.rowOffset][0] < (self.base_TS + self.window_size) and self.isValidLabel(self.curDataChunk[self.rowOffset][0]):
                    #(self.curDataChunk[self.rowOffset][0] >= self.labelStartTs and self.curDataChunk[self.rowOffset][0] <= self.labelEndTs)):
                    self.sensor_data.append(self.curDataChunk[self.rowOffset][:].tolist())
                    self.rowOffset += 1
                    self.totalRowsParsed += 1
            ...
            response = {"sensor_id":self.sensor_id,"base_Timestamp" : self.base_TS ,"data" : self.sensor_data[:], "label": self.label}
            self.base_TS += self.step_size

            # create new segments and store in queue
            while len(self.sensor_data) and self.sensor_data[0][0] < self.base_TS:
                del self.sensor_data[0]

            while (self.dataset_read_completed or 
                            self.curDataChunk[self.rowOffset][0] >= (self.base_TS + self.window_size) or 
                                    (not self.isValidLabel(self.curDataChunk[self.rowOffset][0]))):
        ...
        # check if segments are present in queue
        #if len()
        else:
            print("No more data....")
            return {}
