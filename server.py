...
# Establish connection with client.
    ...
    while True:
        ...
        if len(dir_path):
            ...
            def apply_model(x_train, y_train, x_test, y_test, grp_id):
                ...
                performance["grp_metrics"][int(grp_id)] = {
                ...
                plt.show()
                """
                print(str(grp_id) +"-"+ str(accuracy)+"-"+ str(precision)+"-"+ str(recall))
            ...
            performance["overall_metrics"]["avg_precision"] = np.round(tot_precision / tot_groups, 2)
            performance["overall_metrics"]["avg_recall"] = np.round(tot_recall / tot_groups, 2)
            performance["overall_metrics"]["avg_accuracy"] = np.round(tot_acc / tot_groups, 2)
            print(performance)
            
            c.send(json.dumps(performance).encode())
            """
